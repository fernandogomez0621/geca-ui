...
function DatasetsPage() {
  ...
  const [cvatTasks, setCvatTasks] = useState([]);
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      if (ds) { setSources(ds.sources || []); setReady(ds.ready || []); }
      if (tasks?.tasks) setCvatTasks(tasks.tasks);
      if (mods) setModels(mods.models || []);
      setLoading(false);
    });
  };
  useEffect(load, []);

  const importFromCvat = async (taskId, taskName) => {
    setImporting(taskId);
  ...
  const toggleSource = (name) => {
    setCreateSources(prev => prev.includes(name) ? prev.filter(n => n !== name) : [...prev, name]);
  };

  const createDataset = async () => {
    if (!createName || createSources.length === 0 || creating) return;
    setCreating(true);
  ...
  const loadStats = async (dtype, name) => {
    const r = await api(`/api/datasets/${dtype}/${name}/stats`);
    setStats(r); setViewStats({ dtype, name });
  };

  const deleteDataset = async (dtype, name) => {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div><h1>Datasets</h1><p>Gestión de datasets para entrenamiento</p></div>
        <div style={{display: 'flex', gap: 8}}>
          <button className="btn-primary" onClick={() => setShowCreate(!showCreate)}>+ Crear Dataset</button>
      ...
      {showCreate && (
        <div className="card" style={{marginBottom: 16}}>
          <h3>Crear Dataset para Entrenamiento</h3>
          ...
          <div style={{display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12}}>
            {sources.map(s => (
              <button key={s.folder} className={`btn-sm ${createSources.includes(s.folder) ? 'btn-primary' : 'btn-secondary'}`}
                onClick={() => toggleSource(s.folder)}>
                {createSources.includes(s.folder) ? '✓ ' : ''}{s.folder} ({s.num_images} imgs)
              </button>
            ))}
            {sources.length === 0 && <p className="empty-text">Importa datasets de CVAT primero</p>}
          </div>
          <div className="form-actions">
            <button className="btn-primary btn-sm" onClick={createDataset} disabled={creating || !createName || createSources.length === 0}>
              {creating ? '⏳ Creando...' : `Crear dataset (${createSources.length} sources)`}
            </button>
            <button className="btn-secondary btn-sm" onClick={() => setShowCreate(false)}>Cancelar</button>
          </div>
        </div>
      )}

      {/* READY DATASETS */}
      <h2 style={{fontSize: 16, marginBottom: 12, color: '#00b894'}}>Datasets Listos para Entrenar</h2>
      {ready.length > 0 ? ready.map(d => (
        <div className="card" key={d.folder} style={{marginBottom: 8}}>
          <div style={{display: 'flex', alignItems: 'center', gap: 16}}>
            <div style={{flex: 1}}>
              <h4>{d.name || d.folder} {d.tag && <span className="badge">{d.tag}</span>}</h4>
      ...
      {/* SOURCE DATASETS */}
      <h2 style={{fontSize: 16, marginBottom: 12, marginTop: 24, color: '#6c5ce7'}}>Sources (Importados de CVAT)</h2>
      {sources.length > 0 ? sources.map(s => (
        <div className="card" key={s.folder} style={{marginBottom: 8}}>
          <div style={{display: 'flex', alignItems: 'center', gap: 16}}>
            <div style={{flex: 1}}>
              <h4>{s.task_name || s.folder}</h4>
              <div className="video-meta">
                <span>{s.num_images || 0} imágenes</span><span>·</span>
                <span>{s.num_labels || 0} labels</span><span>·</span>
                <span>{s.class_names?.length || 0} clases</span>
                {s.class_names && <span>· {s.class_names.join(', ')}</span>}
              </div>
            </div>
            <button className="btn-sm btn-secondary" onClick={() => loadStats('sources', s.folder)}>📊 Stats</button>
            <button className="btn-sm btn-danger" onClick={() => deleteDataset('sources', s.folder)}>✕</button>
          </div>
        </div>
      )) : <p className="empty-text" style={{marginBottom: 20}}>Importa tareas anotadas desde CVAT</p>}
        ...
        <div className="card" key={t.id} style={{marginBottom: 8}}>
          <div style={{display: 'flex', alignItems: 'center', gap: 16}}>
            <div style={{flex: 1}}>
              <h4>#{t.id}: {t.name}</h4>
              <div className="video-meta">
                <span>{t.size} frames</span><span>·</span>
                <span>{t.labels?.join(', ') || 'sin labels'}</span><span>·</span>
                <span>{t.status}</span>
              </div>
            </div>
            <button className="btn-sm btn-primary" disabled={importing === t.id}
      ...
      {/* MODELS */}
      <h2 style={{fontSize: 16, marginBottom: 12, marginTop: 24, color: '#ffd43b'}}>Modelos Entrenados</h2>
      {models.length > 0 ? models.map(m => (
        <div className="card" key={m.name} style={{marginBottom: 8}}>
          <div style={{display: 'flex', alignItems: 'center', gap: 16}}>
            <h4>{m.name}</h4>
            <span className="video-meta">{m.size_mb} MB</span>
          </div>
        </div>
      )) : <p className="empty-text">Entrena modelos desde Jupyter usando los datasets listos</p>}
    </div>
  );
}
